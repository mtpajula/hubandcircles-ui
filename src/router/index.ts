...
export const routes: RouteRecordRaw[] = [
  ...
  {
    path: `/:lang/:page(${STATEMENT_PAGES.join('|')})`,
    name: 'statement',
...
// The URL always carries a language and, on the map, a theme (ARKKITEHTUURI.md 4.4, 9). The guard
// fills in and corrects them; an unknown theme falls back to the project default.
router.beforeEach(async (to) => {
  ...
  if (to.name === 'statement') {
    if (lang === param(to.params.lang)) return true
    return { name: to.name, params: { ...to.params, lang }, query: to.query, replace: true }
  }
